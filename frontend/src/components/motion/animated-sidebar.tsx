import { AnimatedSidebarContext, useAnimatedSidebar } from "./sidebar-context"
...
function persistedSidebarOpen(fallback: boolean) {
...
>(function AnimatedSidebarInset({ className, ...props }, forwardedRef) {
  const { isMobile, openMobile } = useAnimatedSidebar()
  return (
    <motion.main
      {...props}
...
>(function AnimatedSidebarContent({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-x-hidden overflow-y-auto overscroll-contain px-2 py-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden",
        className
      )}
    />
  )
...
export function AnimatedSidebarMenuSubButton({
  ...
  const select = (
    ...
    if (
      ...
      return
    if (href && onSelect) event.preventDefault()
    onSelect?.()
...
export function AnimatedSidebarMenuButton({
  ...
  const select = (
    ...
    if (
      ...
      return
    if (href && onSelect) event.preventDefault()
    // In the icon rail, the first click reveals the group instead of toggling
    // an already-open submenu closed before it can become visible.
    if (ariaExpanded !== undefined && panel.collapsed && !context.isMobile) {
      context.setOpen(true)
      if (!ariaExpanded) onSelect?.()
      return
    }
    onSelect?.()
